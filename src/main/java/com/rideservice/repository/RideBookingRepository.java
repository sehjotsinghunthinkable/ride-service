...
public interface RideBookingRepository extends JpaRepository<RideBooking, Long> {
    ...
    @Query("SELECT COALESCE(SUM(b.seatsBooked), 0) FROM RideBooking b " +
    ...
    @Query("SELECT b FROM RideBooking b " +
            "WHERE b.status = 'RESERVED' " +
            "AND b.expiresAt < :now")
    List<RideBooking> findExpiredReservations(@Param("now") LocalDateTime now);
    ...
    List<RideBooking> findAndLockExpiredReservations(@Param("now") LocalDateTime now);

    // Bulk update expired reservations
    @Modifying
    @Query("UPDATE RideBooking b SET b.status = 'EXPIRED', b.version = b.version + 1 " +
            "WHERE b.status = 'RESERVED' AND b.expiresAt < :now")
    int bulkExpireReservations(@Param("now") LocalDateTime now);

    @Modifying
    @Query("UPDATE RideBooking b SET b.status = :status WHERE b.bookingUuid = :bookingUuid")
    ...
    @Modifying
    @Query("UPDATE RideBooking b SET b.status = :status, b.version = b.version + 1 " +
